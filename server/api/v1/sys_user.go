...
func Login(c *gin.Context) {
	...
	if store.Verify(L.CaptchaId, L.Captcha, true) {
		...
		if err, user := service.Login(U); err != nil {
			...
			tokenNext(c, *user, global.GVA_CONFIG.JWT.ExpiresTime)
		}
	} else {
...
// @Success 200 {string} string "{"success":true,"data":{},"msg":"登陆成功"}"
// @Router /base/authenticate [post]
func Authenticate(c *gin.Context) {
	var L request.Login
	_ = c.ShouldBindJSON(&L)
	if err := utils.Verify(L, utils.AuthenticateVerify); err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	...
	if err, user := service.Login(U); err != nil {
		...
		tokenNext(c, *user, 999999999999)
	}

}

// 登录以后签发jwt
func tokenNext(c *gin.Context, user model.SysUser, expiresTime int64) {
