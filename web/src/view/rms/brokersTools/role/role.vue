...
export default {
  ...
  methods: {
      ...
      async onDelete() {
        ...
        const res = await deleteRoleByIds({ ids })
        if (res.code == 0) {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    async updateRole(row) {
      const res = await findRole({ ID: row.ID });
      this.type = "update";
      if (res.code == 0) {
        this.formData = res.data.reRole;
    ...
    async enterDialog() {
      ...
      switch (this.type) {
        case "create":
          res = await createRole(this.formData);
          break;
        case "update":
